...
{0} [-h|help] [options]
    ...
    stop                 توقف {bin}
    restart              ریستارت {bin}
    status               استاتوس {bin} 运行状态
    new                  جدید{bin} jsonپیکربندی
    update               آپدیت {bin} 到最新Release版本
    update [version]     اپدیت نسخه{bin} 到指定版本
    update.sh            更新 multi-v2ray 到最新版本
    add                  اضافه کردن
    add [protocol]       اضافه کردن پروتکل, 端口随机, 如 {bin} add utp 为新增utp协议
    del                  حذف
    info                 اینفو
    port                 پورت
    tls                  修改tls
