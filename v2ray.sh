...
#مسیری که اسکریپت برای اولین بار اجرا شد را ثبت کنید
begin_path=$(pwd)

#روش نصب، 0 یک نصب جدید است، 1 برای حفظ آپدیت پیکربندی v2ray است
install_way=0

#متغیر عامل را تعریف کنید، 0 به معنای خیر، 1 به معنای بله است
help=0
...
#Centos یک نام مستعار را به طور موقت لغو کنید
...
removeV2Ray() {
    #اسکریپت V2ray را حذف کنید
    bash <(curl -L -s https://multi.netlify.app/go.sh) --remove >/dev/null 2>&1
    rm -rf /etc/v2ray >/dev/null 2>&1
    rm -rf /var/log/v2ray >/dev/null 2>&1

    #اسکریپت Xray را حذف نصب کنید
    bash <(curl -L -s https://multi.netlify.app/go.sh) --remove -x >/dev/null 2>&1
    rm -rf /etc/xray >/dev/null 2>&1
    rm -rf /var/log/xray >/dev/null 2>&1

    #قوانین iptable مربوط به v2ray را پاک کنید
    bash <(curl -L -s $clean_iptables_shell)

    #multi-v2ray را حذف کنید
    ...
    #وظیفه به روز رسانی برنامه ریزی شده v2ray را حذف کنید
    ...
    #متغیرهای محیط multi-v2ray را حذف کنید
...
checkSys() {
    #بررسی کنید که آیا Root است یا خیر
...
EOF
    ...
    #اسکریپت قدیمی v2ray bash_completion را حذف کنید
    ...
    #اسکریپت v2ray bash_completion به روز شد
...
profileInit() {
    ...
    #متغیرهای محیط ماژول v2ray را پاک کنید
    [[ $(grep v2ray ~/$env_file) ]] && sed -i '/v2ray/d' ~/$env_file && source ~/$env_file

    #حل مشکل نمایش چینی Python3
    [[ -z $(grep PYTHONIOENCODING=utf-8 ~/$env_file) ]] && echo "export PYTHONIOENCODING=utf-8" >> ~/$env_file && source ~/$env_file

    #پیکربندی جدید برای نصب تازه
